...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Received {len(servants_data)} records. Hydrating database...")
        ...
        for item in servants_data:
            ...
            # 4. Realistic Dynamic Parameters derived from Saint Graph Profile
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"[SYSTEM OK] Database refreshed! Created: {created_count} | Updated: {updated_count}"
            )
        )